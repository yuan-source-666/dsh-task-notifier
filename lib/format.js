 ...
 */
export function resolveTemplate(locale, configured) {
 ...
 */
export function resolveTitles(locale, configured) {
...
export function kindLabel(locale, kind) {
    return labelsFor(locale).kind[kind] ?? kind;
}
/** Localized label for a terminal outcome. */
export function outcomeLabel(locale, outcome) {
...
export function oneLine(text, maxChars) {
    ...
    if (folded.length === 0)
        return '';
    if (maxChars <= 0 || folded.length <= maxChars)
        return folded;
...
export function renderTemplate(template, fields) {
    return template.replace(/\{(\w+)\}/g, (match, key) => (key in fields ? fields[key] : match));
}
/**
 ...
 */
export function renderMessage(input, template, titles, locale, maxChars) {
